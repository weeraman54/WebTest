...
export class ProductService {
  ...
  static async fetchWebsiteProducts(): Promise<Product[]> {
    try {
      // Try inventory_items_public first
      ...
      if (error && error.code === '42P01') {
        console.warn('inventory_items_public not found, using direct table access');
  ...
  /**
   * Fetch sale products (products with sale_price) from inventory view (with fallback)
   */
  static async fetchSaleProducts(): Promise<Product[]> {
    try {
      // Try inventory_items_public first
      let { data, error } = await supabase
        .from('inventory_items_public')
  ...
  static async fetchProductsByCategory(category: string): Promise<Product[]> {
    try {
      // Try inventory_items_public first - use category_name instead of category
      let { data, error } = await supabase
        .from('inventory_items_public')
        .select('*')
  ...
  /**
   * Fetch single product by ID from inventory view (with fallback)
   */
